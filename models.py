...
# Load a small graph dataset (Karate Club)


class CurveAttention(torch.nn.Module):
...
class Model(MessagePassing):
    def __init__(self, gamma, phi):
        ...
        # size of nodes depends on the input graph
        self.embed_dict = nn.Parameter(torch.FloatTensor(725, size_emb_dict))
        self.curve_attention = CurveAttention(16, 8, out_channels, 2)
        self.fuze = nn.Linear(2 * out_channels, out_channels)

    def forward(self, u, edge_index, rel_pos, x_e, L1, x_tri, L2, B1, B2, Curve):
        tmp_2 = self.curve_attention.forward(Curve, edge_index)

        tmp = self.propagate(edge_index, u=u, rel_pos=rel_pos,
                             x_e=x_e, L1=L1, x_tri=x_tri, L2=L2, B1=B1, B2=B2)

        return self.fuze(torch.cat([tmp, tmp_2]))
    ...
    def update(self, aggr, u, x_e, L1, x_tri, L2, B1, B2):
        ...
        supports = F.softmax(
            F.relu(torch.mm(self.embed_dict, self.embed_dict.transpose(0, 1))), dim=1)
        # powers of supports
        # sec_hop_supports = torch.mm(supports, supports)
        # third_hop_supports = torch.mm(supports, sec_hop_supports)
        # 0.5 * supports + 0.3 * sec_hop_supports + 0.2 * third_hop_supports
        final_supports = supports
        adp_node_dudt = self.mlp_adp(
            torch.einsum('mm,md->md', final_supports, u))
        ...
        triangle_dudt_trans = torch.einsum(
            'nm,md->nd', B1.T, triangle_dudt_trans_)
