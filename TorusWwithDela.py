...
class Torus:
    def __init__(self, Distortion = DistortionID):
        ...
        for i in range(2):
            ...
            for j in range(2):
                self.g[-1].append(0)
                for k in range(3):
                    self.g[-1][-1] += sp.diff( self.z[k], self.w[i] ) * sp.diff( self.z[k], self.w[j] )  
                    self.g[-1][-1] = sp.simplify(self.g[-1][-1])
        self.Rieman = SymbolicRiemanFromMetric(self.g, self.w, self.w)


    def ToZ(self, lhsw):
        lhs = [ self.z[i].subs(self.w[0], lhsw[0]).subs(self.w[1], lhsw[1]) for i in range(3) ] 
        return lhs
                ...
                [ 
                    [ 
                        [
                            ...
                            ).subs(self.w[0],pt[0]).subs(self.w[1],pt[1]))
    ...
    def EspilonNet(self,r):
        N = int(10000 / r)
        pc = [ [ 2 * math.pi * random.random(), 2 * math.pi * random.random() ] for i in range(N) ]
        net = []
        while pc:
            curr = pc.pop(0)
            net.append(curr)
    ...
    def Dataset(self, r):
        pc = self.EspilonNet(self,r)
        pcz = [ self.ToZ( pt ) for pt in pc ]
        rms = [ self.ReimanZ(pt) for pt in pc]
        return pc, pcz, Delaunay(points), rms
